---
title: 'Solutions des exercices: probabilités et statistiques pour la biologie'
...
result <- data.frame(k=k, "Proba.match"=p.match)
kable(format(result, digits=3))
```
...
```{r NGS_read_mapping_with_mismatches}
######## NGS read mapping with mismatches ########

## Parameters
p <- 0.25 # Matching probability per nucleotide
k <- 35   # Read length
m <- 3    # Max number of mismatches
L <- 50e+6  # Library size (number of reads)
G <- 3e+9   # Genome size
C <- 23     # Number of cromosomes

# Nominal P-value, i.e. probability to obtain a hit by chance for a given read at a given genomic position
pval <- pbinom(q=k-m - 1, prob=p, size=k, lower=FALSE)

# Number of alignable positons on the genome for a read of length k
N <- 2*(G - C * (k - 1))

# E-value for the whole library
eval <- pval * N * L
```
...
$$
\begin{aligned}
P = P(M \le 3) = P(X \ge k - 3) &= \sum_{i=k-3}^{k} C_k^i \cdot p^i \cdot  (1-p) ^{k-i} \\
 &= \sum_{i=32}^{35} C_{35}^i \cdot 0.25^i \cdot 0.75^{35-i} \\
 &= `r signif(digits=3, pval)`
\end{aligned}
$$
...
Le nombre de faux positifs attendus ($E$) est la p-valeur ($P$) calculée ci-dessus multiplée par le nombre de tests. En effet, on tente d'aligner chacun des $`r L`$ fragments de lecture ("read") sur les 2 brins de chacune des $`r G`$ positions du génome. 

Cependant, il faut tenir compte du fait qu'un read de longueur $k = `r k`$ ne peut pas être aligné sur les $k - 1$ positions d'un chromosome, puisque l'alignement serait incomplet (rappelons qu'on effectue ici des alignements globaux, qui doivent donc couvrir l'ensemble de la longueur du read). Il faut donc soustraire $k - 1 = `r k-1`$ positions sur chaque brin de chaque chromosome, donc au total $2 \cdot C \cdot (k-1)$ positions. 
...
```{r}
k <- 6 ## Length of the GGCGCC restriction site
p <- 1/4  ## Residue probability (equiprobable)
P.site <- p^k ## Prior probability

G <- 4e+6 ## Genome size
exp.sites <- G * P.site ## Number of occurrences expected at random

x <- 130 ## Number of occurrences of the restriction site in the genome
excluded.positions <- x * (k-1)

pval.log <- pbinom(q = x-1, prob = P.site, size = G, lower.tail = TRUE, log.p = TRUE)
pval <- exp(pval.log)
```

$$P(\text{GGCGCC}) = p^k = `r p`^{`r k`} = `r signif(digits=3, P.site)`$$
...
$$E(\text{GGCGCC}) = P(\text{GGCGCC}) \cdot G = `r signif(digits=3, P.site)` \cdot G = `r round(digits=1, exp.sites)`$$

### Probabilité d'observer au plus 130 occurrences

Le nombre d'occurrences observées ($x = 130$) semble nettement inférieur à l'espérance ($E(\text{GGCGCC}) =  `r round(digits=1, exp.sites)`$), c'est-à-dire le nombre attendu étant donné le modèle de background. Nous allons maintenant évaluer la significativité statistique de cette observation en calculant la probabilité d'observer une telle différence par hasard. Si cette probabilité est très faible, on considérera le résultat comme significatif et on tentera d'en fournir une interprétation biologique.

Selon le modèle null (séquences générées au hasard avec des nucléotides équiprobables) on peut modéliser les occurrences d'un site de restriction comme un schéma de Bernoulli: à chaque position génomique, on observe un site (*succès*) ou non (*échec*), avec une probabilité constante $P(\text{GGCGCC}) = `r signif(digits=3, P.site)`$. 


Par simplification, on considérera que la présence/absence d'un site à une position $i$ est indépendante de sa présence/absence à la position $i-1$ (l'undépendance entre essais successifs est une des conditions du modèle de Bernoulli). 

Notons qu'il s'agit là d'une simplification abusive, car si l'on observe un site $\text{GGCGCC}$ à une position donnée, il est impossible d'en observer une autre occurrence sur les 5 positions suivantes. Puisque le génome comporte $x=`r x`$ occurrences du site de restriction, on sait donc qu'il ext exclus de trouver un site sur les $`r k-1` \cdot `r x` = `r (k-1) * x`$ positions qui chevauchent ces instances. Il existe des méthodes permettant de modéliser de telles dépendances, mais elles reposent sur des modélisations trop avancées pour ce cours. Dans l'immédiat, nous adopterons une solution pratique, en considérant que ces `r excluded.positions` positions exclues ne représentent qu'une fraction négligeable du génome ($G = `r G`$).

Après avoir jusitifié (toute mesure gardée) notre modèle de Bernoulli, nous pouvons calculer la probabilité du nombre de sites de restriction avec la **loi binomiale**. Nous calculons ici la surface de la **queue à gauche** de l'observation: $P(X \le `r x`)$, c'est-à-dire la probabilité d'observer entre 0 et `r x` occurrences. 


$$
\begin{aligned}
P(X \le x) &= \sum_{i=0}^{x} C_G^i P(\text{GGCGCC})^i \cdot \left( 1 - P(\text{GGCGCC}) \right)^{G -i}\\
 & = \sum_{i=0}^{`r x`} C_{`r G`}^i \cdot (`r signif(digits=3, P.site)`)^i \cdot \left( 1 - `r signif(digits=3, P.site)`\right)^{`r G` -i} \\
 & = `r signif(digits=3, pval)`
\end{aligned}
 $$

Cette probabilité est extrêmement faible, il serait donc extrêmement improbable d'obtenir un aussi petit nombre d'instances du motif $\text{GGCGCC}$ dans un génome de même taille généré aléatoirement. 

### Interprétation biologique

Le très faible nombre d'occurrences du site $\text{GGCGCC}$ se comprend parfaitement dès qu'on sait que ce site est reconnu par une enzyme de restriction, dont l'activité moléculaire est de cliver l'ADN. Ces enzymes jouent un rôle de protection en fragmentant les molécules d'ADN étrangères au génome de *Escherichia coli*, mais elle pourrait également endommager le génome propre de la bactérie. Le très faible nombre de sites pour sa propre enzyme de restriction résulte d'un effet de contre-sélection de ces sites dans le génome d'*Escherichia coli*. 
...
Le test de Student repose sur deux hypothèses de travail: normalité des données et homoscédasticité.
...
$$t= \frac{m_p-m_t}{s \cdot \sqrt{\frac{1}{n_p} +\frac{1}{n_t}}} = \frac{21-10}{15 \cdot \sqrt{\frac{1}{50} + \frac{1}{50}}} = \frac{11}{\frac{15}{5}}=\frac{11}{3}=3.667$$
...
Note: avec **R** nous pouvons calculer précisément la p-valeur associée à cette valeur de la statistique de Student: $P(t \ge 3.667) = `r 2*signif(digits=3, pt(q=3.677, df=98, lower.tail=FALSE))`$.
...
$$P=P((T \ge 3.667) \lor (T \ge -3.667))= 2 \cdot P(T \ge 3.667)$$
...
Comme le test ne s'applique qu'à un seul gène (celui pour lequel on aeffectué l'expérience de PCR), une valeur $P < 0.001$ peut être considérée comme très significative, et l'on peut rejeter l'hypothèse nulle. On peut donc considérer que ce gène est différentiellement exprimé entre les deux populations.  
...
### Définiton du modèle de Bernoulli

La définition générale du schéma de Bernoulli repose sur une série d'essais (expériences) indpendants, pouvant chacun déboucher sur deux résultats mutuellement exclusifs, respectivement qualifiés de "succès" et "échec", avec une probabilité de succès ($p$) identique pour chaque essai. 

En analyse de séquences, on se base souvent sur un extension du modèle, appelée modèle de Bernoulli généralisé, où chaque essai résulte en un résultat parmi un nombre fini de possibilités. Pour le séquences nucléiques on considère 4 possibilités correspondant aux nucléotides, et pour les séquences peptidiques 20 possibilités correspondant aux acides aminés. Le modèle de Bernoulli généralisé postule que les résidus (nucléotides ou acidés aminés selon le type de séquence) se succèdent de façon indépendante, et que la probabilité d'un résidu donné est la même à chaque position de la séquence. 
...
### Adéquation pour l'analyse des séquences biologiques

Les modèles de Beroulli présentent des qualités pratiques mais il est important de connaître leurs limitations. En effet, l'hypothèse d'indépendance entre résidus successifs n'est pas réaliste pour des séquences macromoléculaires. A titre d'exemples: 

- Dans les génomes de mammifères, le dinucléotde $CG$ (également dénoté $CpG$, où p indique le groupe phosphate) est beaucoup moins fréquent que ce à quoi on s'attendrait sous hypothèse d'indépendance: $F_{CG} < F_C \cdot F_G$. Ceci résulte du fait que ce dinucléotide est fréquemment méthylé par des enzymes spécifiques, qui jouent un rôle dans l'inactivation de l'ADN et la reconnaissance de l'ADN viral. La méthylation de CpG favorise la mutation vers une thymine, par déamination, ce qui a pu susciter une réduction progressive de leur fréquence au cours de l'évolution des mammifères.

- Les séquences non-codantes sont enrichis en oligonucléotides riches en adénines et thymines (poly-AT) par rapport à la fréquence à laquelle on s'attendrait d'après un modèle de Bernoulli.
