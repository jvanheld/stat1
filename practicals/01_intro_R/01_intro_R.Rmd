...
**Remarque : ** R permet également d'utiliser le symbole `=` pour effectuer une assignation. Cependant, nous préférons utiliser l'opérateur explicite `<-`, qui est également préconisé sur les guides de style
...
```{r binomial_series, fig.width = 12, fig.height = 8, out.width = "70%", echo = FALSE}
par(mfrow = c(3,3))
...
```{r fig.width = 10, fig.height = 8, out.width="80%"}
n <- 10
p <- 0.4
...
for (n in c(10, 100, 1000, 1000, 10000, 10000)) {
  ...
  lines(x, dnorm(x, mean = p * n, sd = sqrt(p * (1 - p ) * n)), type="l")
}
par(mfrow=c(1,1))
