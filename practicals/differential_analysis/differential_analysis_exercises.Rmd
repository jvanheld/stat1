...
## Compute some statistics about the samples
...
### How to do run $10,000$ tests efficiently?

Loops are not very efficient in R. However for this course we will use a loop, for the sake of facility. 

In the second semester we will learn a more efficient way of running replicated tasks: organise the data in a table, then define a function that runs one test, and use `apply()` to run the test on each line of the table


Let us generate two data frames

- $X_1$ : $r = `r r`$ samples drawn from the first population
- $X_2$: $r = `r r`$ samples drawn from the second population

```{r generate_data_tables}

## Draw r samples of size n1 from the first population
X1 <- data.frame(
  matrix(nrow = r, ncol = n1, 
         data = rnorm(n = r * n1, 
                      mean = mu1, 
                      sd = sigma1))
)
