...
My filtering function ([R/class_filtering.R]("R/class_filtering.R")) returns a list (conceptually an "object" even though I don't implement it in object-oriented way) that gather all the pieces of information together. 


```{r eval=FALSE}
source("R/class_filtering.R")
#### Filter out the classes having less than 30 individuals ####
filteredData <- filterClasses(x = exprTable, classes = sampleGroup, minPerClass = 30)
...
## Message
message("Spitting data set into train (", 
...
hits <- sum(diag(x = confusionTable))
errors <- sum(confusionTable) - svmHits
MER <- errors / sum(confusionTable) * 100
