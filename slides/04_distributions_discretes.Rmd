...
- La **queue droite** indique la probabilité d'observer un résultat **inférieur ou égal** à une certaine valeur: $\operatorname{P}(X \le x$). 
...
- La **queue gauche** indique la probabilité d'observer un résultat **supérieur ou égal** à une certaine valeur: $\operatorname{P}(X \ge x$).
...
Note: la variance est inférieure à ma moyenne. 


## Distribution binomiale

Quand la probabilité de succès est faible par rapport au nombre d'essais, la distribution prend une forme de $i$. 

```{r fig.width=9, fig.height=4, echo=FALSE, fig.cap="**Distribution binomiale.**  "}

## color specification
if (color.status == "full") {
  plot.colors <- c('probability'='blue',
              'CDF'='darkgreen',
              'dCDF'='purple'
              )
} else {
  plot.colors <- c('probability'='black',
              'CDF'='black',
              'dCDF'='black'
              )
}
...
## Distribution binomiale (suite)

Quand la probabilité de succès relativement élevée mais inférieure à 0.5, la distribution prend une forme en cloche assymétrique. 
...
**Note: ** le recours à la binomiale présuppose un modèlde de Bernoulli. Pour les exemples 2 et 3 ceci revient à considérer que les nucléotides se succèdent de façon indépendante, ce qui est assez peu réaliste. 


## Un modèle de Bernoulli convient-il pour les séquences d'ADN ?
...
```{r echo=FALSE}
oligo.freq <- read.table("../data/oligo_freq/3nt_genomic_Saccharomyces_cerevisiae-ovlp-1str.tab", sep="\t", row.names = NULL)
